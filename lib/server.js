...
module.exports = function resource(primus, options) {
  ...
  primus.resource = function create(name, resource) {
    return this.resources[name] || createResource(name, resource);
  };
  ...
  function createResource(name, resource) {
    ...
    resource.broadcast = function(event, data){
      var args = arguments;
      resource.channel.forEach(function (spark, id, connections) {
    ...
    resource.channel.on('connection', function connection(spark){
      ...
      for (key in resource) {
        if (evRE.test(key)) {
          ev = key.replace('on', '');
          spark.on(ev, resource[key].bind(resource, spark));
          events.push(ev);
        }
      }
